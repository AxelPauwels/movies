<script>
    ...
    function haalMoviesOpNaam(naam) {
        $.ajax({
            type: "GET",
            url: site_url + "/movies/ajaxMoviesOpNaam",
            data: {view: view, zoekstring: naam},
            success: function (result) {
                $("#resultaat").html(result);
                laatsteActie = "haalMoviesOpNaam";
                laatsteVar = naam;
            },
    ...
    function haalMoviesOpJaar(jaar) {
        $.ajax({
            type: "GET",
            url: site_url + "/movies/ajaxMoviesOpJaar",
            data: {view: view, zoekstring: jaar},
            success: function (result) {
                $("#resultaat").html(result);
                laatsteActie = "haalMoviesOpJaar";
                laatsteVar = jaar;
            },
            error: function (xhr, status, error) {
                alert("-- ERROR IN AJAX --\n\n" + xhr.responseText);
    ...
    function haalMoviesOpLaatstToegevoegd(aantal) {
        $.ajax({
            type: "GET",
            url: site_url + "/movies/ajaxMoviesOpLaatstToegevoegd",
            data: {view: view, zoekstring: aantal},
            success: function (result) {
                $("#resultaat").html(result);
                laatsteActie = "haalMoviesOpLaatstToegevoegd";
                laatsteVar = aantal;
            },
            error: function (xhr, status, error) {
                alert("-- ERROR IN AJAX --\n\n" + xhr.responseText);
    ...
    function haalAlleMovies() {
        $.ajax({
            type: "GET",
            url: site_url + "/movies/ajaxAlleMovies",
            data: {view: view},
            success: function (result) {
                $("#resultaat").html(result);
                laatsteActie = "haalAlleMovies";
                laatsteVar = "";
            },
            error: function (xhr, status, error) {
                alert("-- ERROR IN AJAX --\n\n" + xhr.responseText);
    ...
    function haalGesorteerdeMovies(taal, type, duurVan, duurTot, grootteVan, grootteTot, jaarVan, jaarTot) {
        $.ajax({
            type: "GET",
            url: site_url + "/movies/ajaxGesorteerdeMovies",
            data: {
                view: view,
                taalKeuze: taal,
                typeKeuze: type,
            ...
            success: function (result) {
                $("#resultaat").html(result);
                laatsteActie = "haalGesorteerdeMovies";
    ...
    function haalMoviesInfo(id) {
        $.ajax({
            type: "GET",
            url: site_url + "/movies/ajaxMoviesInfo",
            data: {view: view, zoekid: id},
            success: function (result) {
                $("#resultaatModal").html(result);
                $('#myModal').modal('show');
                // attach_download();
                // attach_request(); // is moved to on-ajax-complete
            },
            error: function (xhr, status, error) {
                alert("-- ERROR IN AJAX --\n\n" + xhr.responseText);
            }
    ...
    $(document).ready(function () {
        ...
        inputJaar.keyup(function () {
            if ($(this).val() === "") {
                ...
                haalMoviesOpJaar($(this).val());
            }
        });
        $("#toonRecente").click(function () {
            var aantal = 48;
            haalMoviesOpLaatstToegevoegd(aantal);
        });
        ...
        $("#toonGesorteerd").click(function () {
            ...
            haalGesorteerdeMovies(taal, type, duurVan, duurTot, grootteVan, grootteTot, jaarVan, jaarTot);
        });
    });
...
<div id="defaultSearch" class="container">
    <div class="panel panel-primary">
        <div class="panel-heading">
            ...
            <span class="textShadowSmall">MOVIES</span>
